...
async function deploy(config) {
  const { script, isNpmBuild, webDir, distPath, maxTry, projectName, svnLocalPath } = config;
  ...
  for (const remotePath of webDir) {
    ...
    successLog(`项目部署成功`);
    await pushSvn(distPath, svnLocalPath);
  }
  process.exit(0);
}
...
async function pushSvn(distPath, svnLocalPath) {
  ...
  // await runShellCommand(`svn commit ''`, svnLocalPath);
}
